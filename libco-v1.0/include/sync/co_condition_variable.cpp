...
ConditionVariable::~ConditionVariable() {
  while (!lock_queue_.empty()) {
    auto entry = lock_queue_.pop_front();
    entry->Unref();
  }
}
...
std::cv_status ConditionVariable::wait_impl(
  ...
  AutoFreeHelper<detail::LockEntry> auto_free(lock_entry);
  {
    ...
    lock_queue_.push_back(lock_entry);
    lock_entry->Ref();
  }
  ...
  while (true) {
    ...
    uint8_t s = lock_entry->status.load(std::memory_order_acquire);
  ...
  while (true) {
    {
      ...
      lock_entry = lock_queue_.pop_front();
    }
